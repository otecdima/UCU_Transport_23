...
def filter_data(dataframe):
    ...
    :return:
    """
    # Drop unnecessary stops

    # Drop unnecessary dates_data entries
    pass


def preprocess_data(dataframe, list_stops):
    """
    Prepare the dates_data for one root to be places in the build_heatmap() function
    :param dataframe:
    :return:
    """
    ...
    for stop in bus_stops:
        ...
        for time in work_hours:
            ...
            # Filter the dates_data by vehicle_id (remove duplicates)
...
def build_heatmap(dataframe, route_name):
    """
    Builds heatmap
    :param dataframe:
    :return:
    ...
    layout = go.Layout(
        title=f"Average intervals per bus stop for {route_name}",
        height=900,
        width=1500
    )
...
if __name__ == '__main__':
    ...
    dataframe_stops = pd.read_csv("Zupynky - 3A.csv")  # 3A, 22
    list_stops = dataframe_stops["name"].to_list()

    separate_routes = separate_routes(dataframe)
    preprocessed = preprocess_data(separate_routes[6], list_stops)  # 6, 8
    heatmap = build_heatmap(preprocessed, "3A")
    heatmap.show()
